...
## Session 13 — cross-checking the deliverables, and what the audit turned up

**Date:** 2026-08-30

**Goal:** Audit every deliverable against the problem statement before the deadline, fix
what the audit found, then extend the dashboard: working links to the HTML reports, and an
"Analyse with AI" / "Recommend action" pair in the loan explorer.

**Accepted:**
- **Explicit `encoding="utf-8"` on all sixteen file-I/O sites.** Not one `write_text`,
  `read_text` or `open` in the repo had specified an encoding, so every one inherited
  Windows `cp1252`. Phase 8 was simply the first phase whose content contained a character
  cp1252 cannot encode.
- **`--live-copilot` on `main.py`.** Offline stays the default; live is now *reachable*
  from the main entrypoint, which it previously was not at all.
- **HTML reports served inside the app.** Each report is read from disk, its figures
  inlined as data URIs, and rendered into a sandboxed iframe, with a download button beside
  it.
- **A second grounded ask, sharing one context.** `ACTION_TASK` requests *verification
  steps*; the same guardrails run and the same audit trail receives the call.
- **Streamlit's base theme pinned in `.streamlit/config.toml`**, plus explicit text colours
  everywhere the CSS sets a background.

**Rejected / corrected:**
- *The audit's most damaging finding was not a bug in the code.* `reports/` had been
  cleared and re-run offline, which cut `llm_prompt_log.jsonl` from 208 records — including
  150 live calls against `grok-4` and `openai/gpt-oss-120b` — down to 20 offline stubs.
  The README's claim of a live run was, at that moment, unevidenced in the repository,
  which is the "fabricates results" disqualification condition wearing a friendly face. The
  file was restored from git before anything appended to it. The log is append-only; that
  is not the same as safe.
- *A rule named `nan`.* The first live note from the new explorer button read "The
  validation rule `nan` also fired." A queue row with no triggered rules reads back from
  CSV as float NaN — and **NaN is truthy** — so `triggered or ""` passed it through and
  `str()` made it a rule name. The batch pipeline had the identical bug; it had never
  surfaced because the top six queue rows all happen to have real rules. This is the worst
  class of defect this layer can produce: a fabricated finding generated by *correct,
  grounded* behaviour, which the guardrails cannot catch because the string genuinely was
  in the supplied context. Fixed at `build_context`, the one place every caller funnels
  through, and pinned by a parametrised test.
- *The sidebar was invisible to half its readers.* The CSS set backgrounds without setting
  text colours, so on a machine preferring dark mode Streamlit painted near-white text onto
  the custom near-white sidebar: a contrast ratio of 1.03:1. It had survived every
  screenshot because the development machine was in light mode. Found only by opening the
  app in a browser that was not.
- *Rejected: rendering the reports via a `file://` link or a relative `<a href>`.* Neither
  can work — Streamlit serves from its own origin and never exposes the working directory
  over HTTP, and browsers block `file://` from an http page outright. That is *why* the
  links never redirected; the fix had to be to serve the content, not to correct the URL.
- *Rejected: letting "Recommend action" ask for a recommended outcome.* That is the
  decision guardrail's whole purpose, and routing around it for a nicer-sounding button
  would have made the governance layer decorative. It asks what to **verify**; the
  deterministic rule layer remains what emits a suggested action.

**Verification performed:** 10 new tests (135 across the project); full pipeline re-run
end to end in 531s with Phase 8 live; both explorer buttons exercised against the real
provider in a browser; the inlined report checked in the DOM (840 KB, 5 of 5 figures
loaded as data URIs).

**Human review:** The audit was the human contribution here. Three of the four findings
above — the destroyed audit trail, the invisible sidebar, the fabricated `nan` rule — were
invisible to the test suite and to the code, and surfaced only from reading the artefacts
and the rendered page as a judge would. CI never caught the encoding bug because CI runs
on `ubuntu-latest`, which defaults to UTF-8.
...
## Session 14 — the dashboard as a product

**Date:** 2026-08-30

**Goal:** Reports reachable as real pages, not just embedded; a product identity; the AI
output made readable; and the deliverables checklist re-checked against section 11.

**Accepted:**
- **Reports published to their own URLs.** `enableStaticServing` plus a publish step that
  writes the *self-contained* copy into `static/reports/`. "Open as a page" is now a real
  link with a real URL; "Read inline" and "Download" are the other two routes.
- **A name.** **Bellwether** — the indicator that moves first. Chosen against the servicer
  names the generator invents (Atlas, Northgate, Cornerstone, Beacon, Meridian) so a name
  on the page can never be read as a name in the data. Set in one constant.
- **The AI output rendered as content, not as a dump.** Body as markdown in the reading
  face, verification steps as checkbox rows, the disclaimer stated once below the content,
  and the prompt and `call_id` folded into an expander.
- **The deliverables checklist mirrors section 11 row for row**, including the row that
  fails.
- **One renderer for every piece of generated text.** The copilot page was showing stored
  notes and probe responses as raw markdown in a monospace block -- asterisks where the
  emphasis should be, hyphens where the checklist should be. `_render_generated_text`
  now serves the copilot page and the explorer alike, so a note reads the same wherever
  it appears.
- **Charts beside the model card and the development log.** Read from the same CSVs the
  card was generated from, and -- for the log -- parsed out of the log's own prose, so
  neither can drift away from the document it sits next to.

**Rejected / corrected:**
- *The status badge floated opposite a short heading* with nothing balancing it. Moved
  inline with the sentence it qualifies, where it reads as part of that sentence.
- *The disclaimer was rendered twice around three sentences of content.* `guardrails.wrap`
  repeats it top and bottom on purpose — correct for a text artefact that gets pasted into
  a case file, wrong on screen. The UI now renders `raw_output` and states the label once;
  the wrapped form is untouched and still what the pipeline writes.
- *Rejected: dropping the demo video from the checklist because it does not exist.* It is
  the only section 11 deliverable still outstanding, which is exactly the reason to show
  it. A checklist that lists only what was produced cannot tell anyone what is left.
- *Rejected: making the standalone report follow the app's pinned light theme.* The report
  is also downloaded and read outside the app, its `color-scheme: light dark` is
  deliberate, and every figure already carries its own light surface — verified in the DOM
  rather than assumed.

- *The offline stub nearly lost its own disclaimer.* Stripping the wrapper by "drop
  bracketed lines at the ends" would also have removed
  `[OFFLINE STUB -- not a model response]`, which is content -- presenting a stub as a real
  model generation, the one thing offline mode exists to prevent. Only lines that are
  *wholly* a wrapper and carry the disclaimer or guardrail marker are stripped, and a test
  pins it.

**Verification performed:** 5 further tests (140 total); the published report opened at
`/app/static/reports/data_intelligence_report.html` in a real tab and checked in the DOM
for inlined figures; both AI cards generated live and read on screen.

**Human review:** Every item this session came from looking at the rendered page — a badge
that sat wrong, a disclaimer that read twice, a checklist that flattered itself by omission.
None of it was visible from the code.
...
## Session 15 — documentation for someone who has never seen this

**Date:** 2026-08-30

**Goal:** A README a guest can act on: architecture diagrams, setup for all three
operating systems, screenshots of the UI, and a README in every folder saying what it is
for and what each file in it does.

**Accepted:**
- **Two Mermaid diagrams** — the phase flow with its artefacts, and the module map. The
  first makes the one thing that matters legible at a glance: every arrow into Phase 8
  comes *from* a model, and none comes out of it into a prediction.
- **Per-OS quickstarts** in collapsible blocks — macOS (`brew install libomp`), Debian
  (`libgomp1`), Windows (`.venv\Scripts\`, no OpenMP needed, and a note that `make` is
  not available). Plus a troubleshooting table keyed on the actual error text.
- **`scripts/capture_screenshots.py`.** Screenshots rot faster than any other
  documentation. Making the capture one command is what makes re-shooting happen.
- **Nine folder READMEs**, each documenting every file's responsibility.

**Rejected / corrected:**
- *The first draft of `data/README.md` said the CSVs were gitignored and that a fresh
  clone would find the folder empty.* Both were wrong — the pack is committed. Caught by
  checking `git ls-files data/` instead of trusting the sentence, which is the same
  discipline as checking a metric rather than the code that produced it. Documentation
  gets asserted rather than verified precisely because it is not executable.
- *`models/README.md` was invisible to git.* `.gitignore` had `/models/`, and git does not
  descend into an ignored directory, so the README explaining the directory was ignored
  along with it. Changed to `/models/*` plus `!/models/README.md`. Found by running
  `git check-ignore` over every new file rather than assuming.
- *Screenshots were captured at `device_scale_factor=2` first* — 7.7 MB for eleven images,
  most of a ten-megabyte clone, for detail nobody reads at README width. Re-shot at 1×
  for 3.0 MB.
- *Rejected: adding Playwright to `requirements.txt`.* A judge reproducing the pipeline
  should not have to download a browser engine. It is documented as an optional extra.
- *Removed hardcoded test counts* ("108 regression tests") from the dashboard and README.
  A number that has to be updated by hand every time the suite grows is a number that
  will be wrong.

**Verification performed:** every relative link in all thirteen markdown files resolved
programmatically (0 broken); `git check-ignore` run over every new file; 140 tests still
passing; screenshots regenerated and inspected.

**Human review:** The two errors above were both *plausible sentences* — the kind of
documentation claim that reads as obviously true and is not. Neither would have been
caught by reading the prose.

**Approximate AI-generated code share this session:** ~95% of lines drafted by AI, 100%
human-reviewed.

---

## Session 16 — one README, ten guides

**Date:** 2026-08-30

**Goal:** Replace the nine per-folder READMEs and the 51 KB root README with a single
navigable hub plus one document per topic.

**Accepted:**
- **`README.md` as a hub, not a manual** — 8.7 KB: what this is, a quickstart, a simplified
  architecture diagram, headline results, and a documentation table linking to everything
  else. A newcomer reaches any topic in one click.
- **Ten topic guides in `docs/`** — setup, architecture, pipeline, module reference, data,
  outputs, dashboard, results, design decisions, testing. Each opens with the same
  navigation strip, so moving sideways between them never requires going back to the root.
- **Per-folder READMEs removed.** Documentation next to the code sounds right and is worse
  in practice: `src/README.md` and the root README described the same architecture, and
  the moment they disagreed neither was trustworthy.

**Rejected / corrected:**
- *The split moved documents into `docs/` without moving their links.* Nine references to
  `reports/*` in what became `docs/results.md` silently became wrong — they had been
  correct relative to the repository root and were now relative to `docs/`. Caught by
  resolving every link programmatically rather than clicking a sample.
- *The link checker's first two verdicts were both wrong.* It flagged five anchors that
  were fine, because it collapsed whitespace runs and stripped underscores while GitHub's
  slugger keeps underscores and maps *each* space to its own hyphen -- so
  ``## `models/` — Phase 3`` is `models--phase-3`, not `models-phase-3`. A validator that
  reports false positives gets ignored exactly like a guardrail that does, which is the
  same lesson this project already learned once at the copilot layer.
- *`docs/outputs.md` had five H1 headings* after the merge, and `docs/pipeline.md` two.
  Every heading in the merged sections was demoted one level, skipping code fences so the
  `# comments` inside bash blocks were left alone.

**Verification performed:** every link and anchor in all eleven documents resolved
programmatically (0 broken, 0 bad anchors); em-dash preservation confirmed after the
rewrite; heading structure re-checked; 140 tests still passing.

**Human review:** The relative-link breakage was invisible in the diff -- the link text
never changed, only the file's location did. It took resolving each one against the
filesystem to see it.
...
17. **Evidence can be destroyed by a successful run.** Clearing `reports/` and re-running
    offline cut the LLM audit trail from 208 records to 20 stubs, deleting 150 live calls.
    Every phase reported success; the pipeline was working exactly as designed. An
    append-only log is not a safe log if something upstream is allowed to delete the file,
    and the evidence a rubric grades hardest is the evidence a rerun most easily erases.
18. **"Nothing is untrue" is not "nothing is fabricated."** The copilot reported that a
    validation rule named `nan` had fired. The guardrails passed it, correctly: the string
    really was in the supplied context, because a NaN had been stringified upstream. A
    grounding check verifies that output traces to input. It cannot tell you the input was
    garbage — only reading the generated text can.
19. **Test the environment you are graded in, not the one CI is cheapest in.** The encoding
    bug that halted Phase 8 could never fire on `ubuntu-latest`, and the invisible sidebar
    could never appear on a light-mode development machine. Both were waiting specifically
    for a judge on Windows, or a judge in dark mode.
20. **Some links cannot be fixed as links.** The reports "not redirecting" was not a wrong
    path; Streamlit serves from its own origin and never exposes the working directory, and
    browsers block `file://` from an http page outright. The whole class of fix was
    unavailable, and recognising that is what turned a URL edit into serving the content.
21. **A checklist that only lists what exists is marketing.** Nine of the ten section 11
    deliverables were tracked; the tenth — the demo video — was simply absent from the list,
    so the page reported 9/9 and looked complete. The fix was to add the row that fails.
22. **Correct for a file is not correct for a screen.** The disclaimer is repeated top and
    bottom so it survives a copy-paste into a case file. Rendered in a card, that same
    decision put one sentence twice around three sentences of content. The artefact and its
    display need different treatments of the same text.
23. **Documentation is the one output nobody runs.** Two confident, wrong sentences went
    into `data/README.md` — that the CSVs were gitignored and that a fresh clone would find
    the folder empty — and both read as obviously true. `git ls-files` settled it in
    seconds. Prose deserves the same "check the number, not the code" discipline as a
    metric; it just never fails loudly enough to remind you.
24. **A file that documents an ignored directory is itself ignored.** `.gitignore` had
    `/models/`, so `models/README.md` would have been silently absent from the clone it
    was written for. Git does not descend into an ignored directory. Verified with
    `git check-ignore` over every new file rather than assumed.
25. **Moving a document breaks its links without changing them.** Nine references to
    `reports/*` were correct relative to the repository root and wrong the instant the file
    moved into `docs/`. Nothing in the diff looked different, because nothing in the link
    text was. Relative links need resolving against the filesystem, not reading.
26. **A validator with false positives gets ignored like any other noisy control.** The
    link checker flagged five perfectly good anchors because it collapsed whitespace runs
    and dropped underscores, while GitHub's slugger does neither. Two rounds of "fixing"
    correct links were nearly spent before the checker itself was suspected -- the same
    failure the copilot's grounding check had already taught, in a different costume.
